...
2. Fix the existing v3 migration to hardcode the version (it currently uses `SCHEMA_VERSION` which would now be 4):
```rust
// Change this line in the `if current_version < 3` block:
conn.pragma_update(None, "user_version", SCHEMA_VERSION)?;
// To:
conn.pragma_update(None, "user_version", 3)?;
```

3. Add after the `if current_version < 3` block:
...
Migrates existing v1 patterns into v2 knowledge nodes using the deterministic type mapping from the spec. The function is called from the v4 migration block in `migrate()` — it runs automatically when an existing v3 database is opened for the first time after upgrading.
...
- [ ] **Step 5: Wire into v4 migration block**

In `crates/retro-core/src/db.rs`, add a call to `migrate_patterns_to_nodes` at the end of the `if current_version < 4` block, after the table creation:
...
Run: `cargo test`
Expected: All tests pass, including `test_v4_migration_from_v3` which now also exercises data migration.
...
[dependencies]
retro-core = { path = "../retro-core" }
```

Note: Uses `path` only (no version constraint) since this is a workspace-local dependency. Version will be bumped to 0.4.0 in Task 9 alongside the other crates.
...
- Modify: `crates/retro-cli/Cargo.toml` (version bump to 0.4.0)
- Modify: `crates/retro-projectors/Cargo.toml` (version bump to 0.4.0)
...
Update `crates/retro-projectors/Cargo.toml`:
```toml
version = "0.4.0"
```
...
```bash
git add crates/retro-core/Cargo.toml crates/retro-cli/Cargo.toml crates/retro-projectors/Cargo.toml crates/retro-cli/tests/
