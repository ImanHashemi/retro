...
/// Spawn a `claude` CLI child process, write the prompt to stdin, wait with a timeout,
/// and return the parsed `ClaudeCliOutput`. Shared by `execute()` and `execute_agentic()`.
fn run_claude_child(
