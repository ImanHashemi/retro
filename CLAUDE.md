...
- **AI backend** — sync `AnalysisBackend` trait with `json_schema: Option<&str>` parameter. Primary impl: `claude -p - --output-format json --tools ""` (prompt piped via stdin). `--tools ""` always passed to prevent agent-mode tool calls consuming turns/tokens. JSON-producing calls pass `--json-schema` for constrained decoding (guaranteed valid JSON). YAML-producing calls (skill/agent generation) pass `None`. **CLI quirks**: `--json-schema` requires `--max-turns 2` (uses internal tool call for constrained decoding) and puts output in `structured_output` field (not `result`).
...
- **Pattern discovery quality: DONE** — `--json-schema` structured output on all JSON-producing AI calls (analysis, skill validation, audit) eliminates JSON parse failures via constrained decoding. Session filtering (< 2 user messages = low signal). Explicit directives category ("always"/"never"/"must" at 0.7–0.85 confidence). Pattern accumulation model (single-session at 0.4–0.5, confirmed via updates). Confidence threshold as sole projection gate (removed `times_seen >= 2`). Simplified prompt exclusions. Context-aware analysis prompt. Rolling window analysis (default on, configurable). 114 unit tests.
