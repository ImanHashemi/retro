...
## Description
Tests the full apply → review workflow: `retro apply` generates content and saves it as PendingReview, then `retro review` approves all items and creates a PR on GitHub. Seeds a CLAUDE.md pattern directly in the DB, runs `retro apply` to generate, runs `retro review` with "all:a" to approve, and verifies the PR exists on GitHub with the correct base branch. Cleans up by closing the PR, deleting the remote branch, and removing the worktree.

Uses a git worktree so the user's working tree and branch are never touched.
...
4. Run `git worktree remove /tmp/retro-e2e-worktree 2>/dev/null; git branch -D retro-e2e-base 2>/dev/null; echo "pre-clean done"`
5. Run `RETRO_DEFAULT_BRANCH=$(gh repo view --json defaultBranchRef -q '.defaultBranchRef.name') && git fetch origin $RETRO_DEFAULT_BRANCH 2>&1 && git worktree add -b retro-e2e-base /tmp/retro-e2e-worktree "origin/$RETRO_DEFAULT_BRANCH" 2>&1 && echo "worktree_ready"`

## Steps
1. Run `RETRO_BIN="$(pwd)/target/debug/retro" && unset CLAUDECODE && cd /tmp/retro-e2e-worktree && $RETRO_BIN apply 2>&1`
2. Run `RETRO_BIN="$(pwd)/target/debug/retro" && cd /tmp/retro-e2e-worktree && $RETRO_BIN review --dry-run 2>&1`
3. Run `RETRO_BIN="$(pwd)/target/debug/retro" && cd /tmp/retro-e2e-worktree && echo "all:a" | $RETRO_BIN review 2>&1`
...
8. Run `git worktree remove /tmp/retro-e2e-worktree 2>&1; git branch -D retro-e2e-base 2>&1; echo "cleanup_local_ok"`
